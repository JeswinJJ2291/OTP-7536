...
function(record) {
    ...
    function fieldChanged(scriptContext) {
        ...
        let checkValue = curRec.getValue({
            fieldId: "custentity_jj_apply_coupon" 
        });

        let couponcod = curRec.getField({
            fieldId: "custentity_jj_coupon_code"
        });

        if(checkValue){
            couponcod.isDisabled = false;
            console.log("Coupon Code Field Enabled ");
        }else{
            couponcod.isDisabled = true;
            curRec.setValue({
                fieldId: "custentity_jj_coupon_code",
                value:" ",
    ...
    function saveRecord(scriptContext) {
        ...
        if(checkValue &&(couponcod || couponcod.length !==5)){
